...
void renderCallback(float w, float h, int ssaSub, bool shadowActivated, Illumination illuminationModel, std::string savePath, std::string fileName)
{
	...
	auto ground = std::make_shared<Square>(Vector(0,-1,-11), Vector(90,0,0), 2, material1);
	auto leftWall = std::make_shared<Square>(Vector(-2.1,0,-10), Vector(0,80,0), 2, blue);
	auto rightWall = std::make_shared<Square>(Vector(2.1,0,-10), Vector(0,-80,0), 2, red);
	...
	auto l1 = std::make_shared<PointLight>(Vector(0.2f, -0.7, -9), Vector(0, 0, 0), Color(1, 0, 0), Color(1,0,0), 1.f);
	auto l2 = std::make_shared<PointLight>(Vector(-0.2f, 0, -10), Vector(0, 0, 0), Color(0, 0, 1), Color::white*0.4f, 1.f);
